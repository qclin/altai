export default  class gLife{
  constructor(p, width, height, palette){
    this.p = p;
    this.w = 10;
  ...
  display() {
    ...
    for ( var i = 0; i < this.columns;i++) {
      for ( var j = 0; j < this.rows;j++) {
        var chooseIndex = p.floor(p.random(this.palette.length))
        // p.fill(this.palette[chooseIndex]);
        if ((this.board[i][j] == 1)) p.fill(255);
        else p.noFill();
        // stroke(this.palette[chooseIndex]);
        // strokeWeight(1);
        p.noStroke();
        p.rect(i*this.w, j*this.w, this.w-1, this.w-1);
      }
    }
  ...
  init() {
    for (var i = 0; i < this.columns; i++) {
      for (var j = 0; j < this.rows; j++) {
        ...
        else this.board[i][j] = p.floor(p.random(2));
        this.next[i][j] = 0;
      }
    }
  }
