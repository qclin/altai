...
text = open("altai_stories_brodsky.txt").read().decode('utf-8') #NOTe: path is relative to root
doc = nlp(text)

adverbs = [item.text for item in doc if item.pos_ == 'ADV']
gerunds = [item.text for item in doc if item.tag_ == 'VBG']
entity = [item.text for item in doc.ents if item.label_ == 'PRODUCT']
